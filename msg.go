...
type RPC struct {
	...
	// Managed by the session.  Will be overwritten went sent on the wire.
...
type RPCReply struct {
	...
	RPCErrors []RPCError `xml:"rpc-error,omitempty"`
}
...
func (d *Response) Decode(v any) error {
	return xml.NewDecoder(d).Decode(v)
}
