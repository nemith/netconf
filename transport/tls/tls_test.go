...
func TestTransport_Dial(t *testing.T) {
	...
	w, err := tr.MsgWriter()
	require.NoError(t, err)
	io.WriteString(w, "a man a plan a canal panama")
	w.Close()

	tr.Close() // Send EOF to server
	srv.Wait(t)
